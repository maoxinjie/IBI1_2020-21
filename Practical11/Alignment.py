...
for	i in range(len(seq_h)): 
    if	seq_h[i]!=seq_r[i]:		
        human_random += 1 
    if (seq_h[i], seq_r[i]) in blosum62:
        score2 = score2 + blosum62[seq_h[i], seq_r[i]]
    elif (seq_r[i], seq_h[i]) in blosum62:
        score2 += blosum62[seq_r[i], seq_h[i]]
print (1-human_random/len(seq_h))
print (score2)
for	i in range(len(seq_h)): 
    if	seq_m[i]!=seq_r[i]:		
        mouse_random += 1 
    if (seq_m[i], seq_r[i]) in blosum62:
        score3 = score3 + blosum62[seq_m[i], seq_r[i]]
    elif (seq_r[i], seq_m[i]) in blosum62:
        score3 += blosum62[seq_r[i], seq_m[i]]
print (1-mouse_random/len(seq_h))
print (score3)
